...
resource "aws_lambda_permission" "allow_api_gateway-getPlan" {
  ...
  source_arn = "arn:aws:execute-api:${var.aws_region}:${var.aws_account_id}:${aws_api_gateway_rest_api.DashCamAPI.id}/*/${aws_api_gateway_integration.Plan-integration.integration_http_method}${aws_api_gateway_resource.Plan.path}"
}