...
// Subscriptions - UserPlanByDate
resource "aws_cloudwatch_metric_alarm" "dynamodb-subscriptions-UserPlanByDate-consumed-read" {
  alarm_name = "${var.environment_name}dynamodb-subscriptions-UserPlanByDate-consumed-read"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods = "1"
  metric_name = "ConsumedReadCapacityUnits"
  namespace = "AWS/DynamoDB"
  ...
  threshold = "${aws_dynamodb_table.subscriptions-table.read_capacity * 0.8}"
  treat_missing_data = "notBreaching"
  alarm_description = "DynamoDB table ${aws_dynamodb_table.subscriptions-table.name} GSI UserPlanByDate is consuming 80% of its provisioned read capacity"
  alarm_actions       = ["${aws_sns_topic.slack_alert.arn}"]
  dimensions {
    TableName = "${aws_dynamodb_table.subscriptions-table.name}"
    GlobalSecondaryIndexName = "UserPlanByDate"
  }
}

resource "aws_cloudwatch_metric_alarm" "dynamodb-subscriptions-UserPlanByDate-consumed-write" {
  alarm_name = "${var.environment_name}dynamodb-subscriptions-UserPlanByDate-consumed-write"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods = "1"
  metric_name = "ConsumedWriteCapacityUnits"
  namespace = "AWS/DynamoDB"
  ...
  threshold = "${aws_dynamodb_table.subscriptions-table.write_capacity * 0.8}"
  treat_missing_data = "notBreaching"
  alarm_description = "DynamoDB table ${aws_dynamodb_table.subscriptions-table.name} GSI UserPlanByDate is consuming 80% of its provisioned write capacity"
  alarm_actions       = ["${aws_sns_topic.slack_alert.arn}"]
  dimensions {
    TableName = "${aws_dynamodb_table.subscriptions-table.name}"
    GlobalSecondaryIndexName = "UserPlanByDate"
