...
resource "aws_route53_record" "www_DashCamWeb" {
  ...
  type    = "A"
  alias {
    name                   = "${aws_cloudfront_distribution.www-website_s3_distribution.domain_name}"
    zone_id                = "${aws_cloudfront_distribution.www-website_s3_distribution.hosted_zone_id}"
    evaluate_target_health = true
  }
}